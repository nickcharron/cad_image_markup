...
struct CameraProjectionFunctor {
  ...
  bool operator()(const double* P, double* pixel) const {
    ...
    std::cout<<"CERES: building projection" << std::endl;
...
struct CeresReprojectionCostFunction {
  ...
  bool operator()(const T* const T_CR, T* residuals) const {
    ...
    std::cout << "CERES: Attempting to Evaluate Cost function" << std::endl;
    ...
    std::cout << "CERES: Computing Residuals" << std::endl;
...
template <class T>
using opt = cad_image_markup::optional<T>;
...
struct CameraProjectionFunctor {
  ...
  bool operator()(const double* P, double* pixel) const {
    Eigen::Vector3d P_CAMERA_eig{P[0], P[1], P[2]};
    bool in_image = true;
    bool in_domain = true; // NOTE: Force these true when using the other 
    bool outside_domain = false;
    cad_image_markup::opt<Eigen::Vector2d> pixel_projected =
        camera_model_->ProjectPointPrecise(P_CAMERA_eig, outside_domain);
    ...
    // TODO: convert projection functions to this standard //////////////////////////////////////////////////////////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
private:
  bool checkDomain(const double* P) {
    Eigen::Vector3d P_CAMERA_eig{P[0], P[1], P[2]};
    bool in_image = true; // NOTE: force this true when using these versions of the projections functions
    bool outside_domain = false;
    cad_image_markup::opt<Eigen::Vector2d> pixel_projected =
        camera_model_->ProjectPointPrecise(P_CAMERA_eig, outside_domain);
    return outside_domain;
